...
人間の手で描くイラストレーションを愛しています。「AIで描けるなら人間は絵を描かなくていい」ってことはないと思いたい。
自身もマンガを描いてました。過去形でなく、作品を作るうえではスケッチやキャラクターデザイン、指示書や発明の図など、マンガは業務でも描きます。

しかしそれはそれとして、
「有名AIが描いたクオリティの高い絵を見たい」のではないでしょうか？
少なくとも私は観たいと思います。
そして人間の絵師が「私は有名AIです」といってイラスト投稿サイトで有名になるという未来です。
もう来月ぐらいには一般的になるんじゃないかなと。
そこまで行くと人類は何かを捨ててしまった感じがするので、本稿では「AIとコラボして人気絵師になる」を目標にしてみたいと思います。
...
鉛筆を削ってデッサンを学ぶよりは楽なので付き合ってください。
本稿では「Stable Diffusion を使っていい感じの萌え絵の素材を描かせるためのプロンプト（呪文）」を理解する、という技術の入口と根っこを同時に達成するために、
数式を一切使わずに、思い通りの結果を得るために必要な研究を解説していきたいと思います。
その過程で DALL-E や midJourney 、Stable Diffusion の背景に使われている技術が面白いな、と思えればそれでOKです。

「人工知能で絵を描く」と一言で表現しても、いろいろな要素が出てきました。例えば Photoshop などを長年開発しているAdobe社にはAdobe Research という研究グループがあります。彼らは画像を直感的な作業で改善するというシンプルな出発点から、写真のフィルタリング技術、背景の除去などを長年開発してきました。最近では、動画の音声を文字起こししてくれたり、簡単なレイアウトを描くだけで神々しい背景画像を作り出してくれたり…となかなか便利そうな技術が国際会議やカンファレンスなどでのデモで発表されています。
...
OpenAI は、人工知能を研究する非営利団体です。人類全体に「害をもたらすよりは、有益性があるやりかた」で、オープンソースと親和性の高い人工知能を注意深く推進することを目的としています。
これまで GPT (Generative Pretrained Transformer) のような自然言語処理、言語のメタ学習が可能なアルゴリズムを発表していましたが、最近は DALL-E という「文字列から（意味を解釈して）画像を生成する」AIを発表しています。
...
2022年4月にOpenAIの Aditya Ramesh らによって書かれた「DALL·E 2」についての論文「Hierarchical Text-Conditional Image Generation with CLIP Latents（筆者による邦題；CLIP 潜在性を用いた階層的テキスト条件付き画像生成法）」(https://arxiv.org/abs/2204.06125)を平易な日本語で関連用語とともに解説していきたいと思います（間違っていたらごめんなさい）。

==== Diffusion モデルと関連する専門用語

専門用語がいくつか出てきます、このあとの急速な進化が予感される「AIで絵を描く技術」、特に Stable Diffusion で覚えておきたい用語は「CLIP」と「Diffusionモデル」、そして「GLIDE」でしょうか。
...
まずOpenAIの先行研究として CLIP (Connecting Text and Images) という手法があります。これは「WebImageText」という画像とそれを説明するフリーテキストのペアでできた4億個の巨大な自然言語教師データを使って構築された自然言語教師型画像分類モデルです。多様なタスクにゼロショット、タスク特有の学習阻止内で事前学習データだけで学習したモデルを直接タスクに転用することができるという特徴があります。

CLIP のような既存手法ではセマンティクス（≒意味、この場合は画像の領域に描かれた意味）とスタイルの両方を捉えた頑健な画像表現を学習することできます。
この「意味」と「表現」の2つを画像生成に活用することを考えて、「キャプション」（注釈として与える文字列）を指定してCLIP画像への埋め込みを生成する事前のモデル（prior；事前分布）と、画像埋め込み(embed)を条件として画像を生成する「デコーダ」の2段階によるモデルを提案しています。

つまり画像の表現スタイルを明示的に指定すれば、フォトリアリズムな表現とキャプションで指定された意味の類似性における差を最小限に抑えつつ、画像の多様性を向上させられるという手法です。さらに、CLIP に結合されている埋め込まれた意味空間は、ゼロショットで（つまり事前の知識やゴールの設定なく）言語誘導的な画像操作を可能にします。デコーダには拡散モデル（Diffusion Model）を用い、事前分布には自己回帰モデルと拡散モデルの両方を用いて実験し、後者の方が計算効率が良く、より高品質なサンプルを生成できることを見出しました。

このあとに続く研究でも CLIP が使われています（unCLIPなど多様な種類があります）。
この CLIP が学習したものが、このあとの結果に大きな影響を及ぼします。

==== DDPM: ノイズ除去拡散確率モデル
「Diffusionモデル」は生成手法の一種で、DALL-E2では GLIDE という DiffusionモデルによるText-to-Image生成モデルが使われています。ノイズ除去拡散確率モデル（DDPM: denoising diffusion probabilistic models）というモデルです。

==== 逆マルコフ過程と超解像技術（SR）
DDPMを理解する上では、2015年ぐらいから研究されている超解像（Super Resolution; SR）の技術を理解せねばなりません。
まず画像に微小なノイズを足していく過程を考えます。これは「マルコフ過程」として表現できます。
マルコフ過程とは、未来の挙動が現在の値だけで決定され、過去の挙動と無関係であるという性質を持つ確率過程です。
「各ステップごとに画像データにノイズを加えていく過程」、これは「破損プロセス」とある現在の画像に対してノイズを加えていくのであり、それぞれのステップは過去のすべての行為とは無関係といえます。
DDPMでは、このような思い切った考えで、ノイズから実データを復元するようにモデルを学習させていきます。
ノイズはガウス分布という一般的な発生の確率に沿わせます。
...
超解像技術を用いたノイズ除去拡散確率モデル条件付き画像生成は、確率的ノイズ除去処理によって超解像、つまり本来なかった倍率まで解像度を上げていくという技術ですが、特に顔や自然画像に対して高い性能を発揮します。

そして「なぜこのノイズの除去で画像が生成されるのか？」という点については、また別途研究されていて（わかっていないことも多かった）、ガウスノイズ以外のノイズでも動作することが報告されています。
例えばColdモデル、つまりランダム性がないDiffusionモデルも存在します。

 * Cold Diffusion: Inverting Arbitrary Image Transforms Without Noise (https://arxiv.org/abs/2208.09392)

=== 潜在変数と拡散モデルを使った描画
超解像技術の場合は「もともと描かれていた画像に対して、解像度を上げていく」という条件付けですが、この潜在変数を CLIPの潜在性 (CLIP Latents)に注目し、画像と言語の関係に埋め込まれた意味（セマンティック）にしたものが GLIDE です。

35億パラメータで、このDiffusion(拡散)モデルを学習し、もともとの意味としての画像は64x64ピクセルのジグソーパズルのようなピースでのセマンティックで作ります。
このセマンティックは「デコードするといい感じの画像を生成する潜在変数」です。
...
なお、レイアウト上、複雑になるとうまく生成できない問題がありますが、領域を指定することで人間がさらに補っていくことで完成させるという手法も並列して存在します。
過度な脱線を避けるため、本稿ではワンショットで、領域指定をしない手法を中心に紹介していきます。
...
画像の埋め込みを生成する「事前分布 (prior)」と、埋め込みから画像自体を生成する「デコーダー (decoder)」によって、構成されます。
自然言語による記述文と、「デコーダー」は、CLIP 画像埋め込みから生成します。
画像を説明する説明文が言語モデルを介して「embedded特徴」に変換されます。
学習と生成は以前と同じように進行しますが、ノイズ除去の追加入力としてヒントを提供するテキスト入力が入ります。
画像を受け取ってembedded特徴を生成するので、「CLIP を反転」しているものだと考えればよいかもしれません。
...
そもそも ``REALITY`` が何のことなのか、知っているのかどうか、これは難しいワードだと思って試しています。
...
まずは、裏側で動いているアルゴリズムやモデルが謎すぎます。おそらくCLIPなのですが、学習している対象が日本のアニメやマンガではなく、映画やスーパーレアリズムが多く、インパクトのある背景画は生成できますが、安定した萌え絵には使いづらい、というか出てくる人物がいつも同じなので、『ああ。MidJourneyだね』という感想を抱いてしまうと思います。

またモデルの改善が期待されますが、改善されることで結果も変わってしまうので、「企画者としてのコンセプト画像の検索」という使い道ではいけるように感じました。

=== MidJounery が切り拓いた「AI画像生成というエンタメ」

仕事には使えないかもですが、
いずれにせよ、MidJourneyはエンタメ体験として成立ている印象を得ました。
GPUやdiscordを使った処理の速さも重要で、お金払う価値はあるかも、という体感がありますし、何より周りの人たちが一生懸命画像を生成しているのを見るのは、なんだか熱狂を感じます。

ところで攻略したいゲーマー(人類)にとってお絵描きAIは何者なのでしょうか。
